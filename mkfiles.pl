...
sub mfval($) {
    ...
    if (grep { $type eq $_ }
	("vc","vcproj","cygwin","borland","lcc","gtk","mpw","nestedvm","osx","wce","gnustep","emcc","emscripten")) {
	    return 1;
	}
...
if (defined $makefiles{'emscripten'}) {
    $mftyp = 'emscripten';
    $dirpfx = &dirpfx($makefiles{'emscripten'}, "/");

    ##-- Emscripten makefile
    open OUT, ">$makefiles{'emscripten'}"; select OUT;
    print
    "# Makefile for $project_name under emscripten.\n".
    "#\n# This file was created by `mkfiles.pl' from the `Recipe' file.\n".
    "# DO NOT EDIT THIS FILE DIRECTLY; edit Recipe or mkfiles.pl instead.\n";
    # gcc command line option is -D not /D
    ($_ = $help) =~ s/=\/D/=-D/gs;
    ...
    foreach $p (&prognames("MX")) {
      ($prog, $type) = split ",", $p;
      $objstr = &objects($p, "X.em.o", undef, undef);
      $objstr =~ s/osx\.em\.o/emscripten\.em\.o/;
      print &splitline("\$(BINPREFIX)" . $prog . ".js: emscripten.js emscripten-pre.js " . $objstr), "\n";
      $libstr = &objects($p, undef, undef, "-lX");
      print &splitline("\tEMCC_CLOSURE_ARGS=\"--compilation_level SIMPLE_OPTIMIZATIONS\" \$(CC) -s ALLOW_MEMORY_GROWTH=1 -o \$@ $objstr $libstr --jcache --post-js emscripten.js --pre-js emscripten-pre.js -O2", 69),
	  "\n\n";
    }
    foreach $d (&deps("X.em.o", undef, $dirpfx, "/")) {
      $oobjs = $d->{obj};
      $ddeps= join " ", @{$d->{deps}};
      $oobjs =~ s/osx/emscripten/g;
      $ddeps =~ s/osx\.m/emscripten.c/g;
      print &splitline(sprintf("%s: %s", $oobjs, $ddeps)),
          "\n";
      $deflist = join "", map { " -D$_" } @{$d->{defs}};
      print "\t\$(CC) \$(COMPAT) \$(FWHACK) \$(CFLAGS) \$(XFLAGS)$deflist" .
